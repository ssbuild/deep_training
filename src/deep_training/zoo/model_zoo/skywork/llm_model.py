...
from deep_training.models.models.skywork.modeling_skywork import SkyworkForCausalLM,SkyworkConfig,setup_model_profile
...
from .tokenization_skywork import SkyworkTokenizer
import logging
logger = logging.getLogger(__name__)


class MySkyworkForCausalLM(SkyworkForCausalLM):
...
class TransformerForLM(TransformerBase):
    def __init__(self, *args,**kwargs):
        ...
        self.set_model(self.from_pretrained(MySkyworkForCausalLM, *args, **kwargs))
...
class MyTransformer(TransformerForLM, ModelWeightMixin,BaseModelWrapper, with_pl=True):
    ...
    def get_llm_model(self) -> MySkyworkForCausalLM:
        if self.lora_args is not None and self.lora_args.enable:
