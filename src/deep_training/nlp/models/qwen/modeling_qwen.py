...
class QWenAttention(nn.Module):
    def __init__(self, config, layer_number=None,**kwargs):
        super().__init__()
...
class QWenBlock(nn.Module):
    def __init__(self, config, layer_idx=None, num_expert=1,**kwargs):
        super().__init__()
...
class QWenModel(QWenPreTrainedModel):
    ...
    def __init__(self, config: QWenConfig,**kwargs):
        ...
        if self.position_embedding_type == "learned":
            self.wpe = nn.Embedding(max_sequence_length, self.embed_dim,**kwargs)
...
class RotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, base=10000,**kwargs):
        ...
        self.dim = dim
        self.base = base
        self.inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2,**kwargs).float() / dim))
