...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig,**kwargs):
        ...
        self.q_proj = init_method(nn.Linear,self.hidden_size, self.num_heads * self.head_dim, bias=bias,**kwargs)
        self.k_proj = init_method(nn.Linear,self.hidden_size, self.num_key_value_heads * self.head_dim, bias=bias,**kwargs)
        self.v_proj = init_method(nn.Linear,self.hidden_size, self.num_key_value_heads * self.head_dim, bias=bias,**kwargs)
        self.o_proj = init_method(nn.Linear,self.num_heads * self.head_dim, self.hidden_size, bias=bias,**kwargs)
        self._init_rope()
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig,**kwargs):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = (
            LlamaAttention(config=config,**kwargs)
            if not getattr(config, "_flash_attn_2_enabled", False)
            else LlamaFlashAttention2(config=config)
        )
        global skip_init_function
        init_method = skip_init_function
        self.mlp = init_method(LlamaMLP,config,**kwargs)
