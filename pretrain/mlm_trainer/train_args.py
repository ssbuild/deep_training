...
def build_args():
    ...
    train = parser.add_argument_group(title='train_args')
    data = parser.add_argument_group(title='data_args')

    train_args(train,False)

    data_args(data,False)

    return preprocess_args(parser.parse_args())

def train_args(parser = argparse.ArgumentParser(),is_build=True):
    """设置训练模型所需参数"""
    ...
    parser.add_argument('--train_file', default='./data_dir/train_data.json', type=str, help='Input raw text file (or comma-separated list of files)')
    parser.add_argument('--eval_file', default=None, type=str,help='Input raw text file (or comma-separated list of files)')
    parser.add_argument('--test_file', default=None, type=str, help='Input raw text file (or comma-separated list of files)')
    parser.add_argument('--label_file', default=None, type=str, help='Input raw text file (or comma-separated list of files)')
...
def data_args(parser = argparse.ArgumentParser(),is_build=True):
    ...
    parser.add_argument('--random_seed', default=42, type=int, help='Random seed for data generation.')
    parser.add_argument('--dupe_factor', default=5, type=int, help='Number of times to duplicate the input data (with different masks).')
